// Ejercicios Propuestos por https://aprendejavascript.org/  y resueltos por Arodrig
...
function myFunction(str,n){
    ...
    }else{ return 'the parameters must be a string and a number'
}}
...
function myFunction(arguments){
    const re = /xyz/gi;
    const reOne = /, /gi;
    arguments = arguments.replace(re, ' ').replace(reOne, '')
    return arguments;
}

    /* 9) Programa una función que obtenga un numero aleatorio entre 501 y 600 */
const myFunction =  () => Math.ceil(Math.random()*100 + 500)
...
function myFunction(n){
    ...
    function binaryToDecimal(n,base) {
        let v,result;
        let sum =0;
        let arr = [];
...
const percentage = (n,m) => n- (n*m/100)
